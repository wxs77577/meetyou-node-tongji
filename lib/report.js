...
module.exports = class ReportService {
  ...
  getSiteList () {
    ...
    logger.info('[BAIDU_TJ] 获取网站列表 for user', _.get(this.postHeader, 'username'));
    return new Promise((resolve,reject)=>{
        ...
        .then((res)=>{
          let header = res.header;
            logger.info('-------------------------------------------------------------');
            logger.info(`[BAIDU_TJ] 请求次数还剩：${header.rquota}`);
            logger.info('-------------------------------------------------------------');
          ...
          if (resStatus === 0 && _.isArray(resData)) {
              logger.info('[BAIDU_TJ] 获取网站列表成功:', JSON.stringify(resData));
              const siteList = []
      ...
      return new Promise((resolve,reject)=>{
          fetch(_url.getData, {
            ...
            .then((res)=>{
                ...
                logger.info('-------------------------------------------------------------');
                logger.info(`[BAIDU_TJ] 请求次数还剩：${response.header.rquota}`);
                logger.info('-------------------------------------------------------------');
                resolve(response);
            })
            .catch((err)=>{
